...
class KeralaCompose(BaseGazette):
    ...
    def make_dwr_call(self, script_name, method_name, cookiejar, curr_url, dateobj, \
        ...
        if sessionid == None and need_session == True:
    ...
    def extraordinary_get_dwr_post_args(self, dateobj):
    ...
    def ordinary_get_dwr_post_args(self, dateobj):
        datestr = dateobj.strftime('%Y-%m-%d')
        year = dateobj.year
        args = {
            'c0-param0'  : f'string:{year}',
    ...
    def extraordinary_get_metainfos(self, results, dateobj):
        minfos = []
    ...
    def ordinary_get_metainfos(self, results, dateobj):
        minfos = []
        for result in results:
    ...
    def download_onetype(self, relpath, dateobj, get_dwr_post_args, get_metainfos):
        dls = []
        ...
        postargs = get_dwr_post_args(dateobj)
        ...
        metainfos = get_metainfos(results, dateobj)

        for metainfo in metainfos:
            num = metainfo.pop('num')

            formdata = self.replace_field(formdata, 'searchpdfid', num)
    ...
    def download_oneday(self, relpath, dateobj):
        edls = self.download_onetype(relpath, dateobj, self.extraordinary_get_dwr_post_args, self.extraordinary_get_metainfos)
        odls = self.download_onetype(relpath, dateobj, self.ordinary_get_dwr_post_args, self.ordinary_get_metainfos)
        return edls + odls
