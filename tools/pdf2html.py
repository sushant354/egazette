...
``<datadir>/raw/central_extraordinary/2026-01-01/269031.pdf`` and the HTML is
written to ``<datadir>/html/central_extraordinary/2026-01-01/269031.html``.
...
Run from the directory that contains the ``egazette`` package, e.g.:
    ...
    python -m egazette.tools.pdf2html -D /home/sushant/gzdl -e pymupdf \
            -s central_extraordinary -t 01-01-2026 -T 31-01-2026
...
def convert(storage, htmldir, engine, legallayout_dir, relurl_pdfs, overwrite):
...
def main():
    ...
    htmldir = os.path.join(args.datadir, 'html')
    ...
    convert(storage, htmldir, args.engine, args.legallayout_dir,
            relurl_pdfs, args.overwrite)
