...
class MadhyaPradesh(BaseGazette):
    ...
    def download_ordinary(self, dls, relpath, dateobj):
        for partnum, parturl in self.ordinary_urls:    
            parturl = urllib.basejoin(self.baseurl, parturl % dateobj.year)
    ...
    def download_extraordinary(self, dls, relpath, dateobj):
        ex_url = urllib.basejoin(self.baseurl, self.extraordinary_url % dateobj.year)
    ...
    def find_result_table(self, d):        
        ...
        tablelist.sort(cmp = lambda x, y: cmp(x[1], y[1]), reverse = True)
        if not tablelist:        
            return None
    ...
    def find_result_order(self, tr):
        ...
        for td in tr.find_all('th'):
            ...
            if txt and re.search(u'\u0935\u093f\u0937\u092f', txt):
                order.append('subject')
            elif txt and re.search(u'\u0935\u093f\u092d\u093e\u0917', txt):
                order.append('department')
            elif txt and re.search(u'\u0926\u093f\u0928\u093e\u0902\u0915', txt):
                order.append('date')
            elif txt and re.search(u'\u0915\u094d\u0930\.', txt):
                order.append('gznum')
            else:
                order.append('')
